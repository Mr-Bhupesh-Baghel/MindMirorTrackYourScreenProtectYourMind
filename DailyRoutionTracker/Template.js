const sections = {
    ...
    function loadTasks() {
      ...
      // Load custom tasks
      const custom = JSON.parse(localStorage.getItem("customTasks-" + today) || "[]");
      if (custom.length) {
    ...
    function addCustomTask() {
      ...
      if (!task) return;
      const list = JSON.parse(localStorage.getItem("customTasks-" + today) || "[]");
      list.push(task);
      localStorage.setItem("customTasks-" + today, JSON.stringify(list));
      input.value = '';
      loadTasks();
      loadStatus();
    }
    ...
    function autoReset() {
      ...
      if (lastDate !== today) {
        ...
        localStorage.setItem("customTasks-" + today, "[]");
      }
    }
